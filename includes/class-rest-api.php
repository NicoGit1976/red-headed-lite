 ...
 *
 * Routes:
 *   GET    /red-headed-lite/v1/profiles
 *   GET    /red-headed-lite/v1/profiles/{id}
 *   POST   /red-headed-lite/v1/profiles
 *   DELETE /red-headed-lite/v1/profiles/{id}
 *   POST   /red-headed-lite/v1/profiles/{id}/run     → fires an export, returns job
 *   GET    /red-headed-lite/v1/jobs                  → list with pagination
 *   GET    /red-headed-lite/v1/jobs/{id}
 *   GET    /red-headed-lite/v1/jobs/{id}/download    → signed PDF/CSV/etc.
 *
 * @package Red_Headed_Lite
 */
class Red_Headed_REST_API {
    const NS = 'red-headed-lite/v1';
    ...
    public static function get_profile( $req ) {
        ...
        return $p ? rest_ensure_response( $p ) : new \WP_Error( 'not_found', __( 'Profile not found.', 'red-headed-lite' ), array( 'status' => 404 ) );
    }
    ...
    public static function run_profile( $req ) {
        $p = Red_Headed_Profile_Repo::get( (int) $req->get_param( 'id' ) );
        if ( ! $p ) return new \WP_Error( 'not_found', __( 'Profile not found.', 'red-headed-lite' ), array( 'status' => 404 ) );
    ...
    public static function get_job( $req ) {
        ...
        return $row ? rest_ensure_response( $row ) : new \WP_Error( 'not_found', __( 'Job not found.', 'red-headed-lite' ), array( 'status' => 404 ) );
    }
    public static function download_job( $req ) {
        global $wpdb;
        $tbl = $wpdb->prefix . 'rh_jobs';
        $row = $wpdb->get_row( $wpdb->prepare( 'SELECT * FROM ' . $tbl . ' WHERE id = %d', (int) $req->get_param( 'id' ) ), ARRAY_A );
        if ( ! $row ) return new \WP_Error( 'not_found', __( 'Job not found.', 'red-headed-lite' ), array( 'status' => 404 ) );
        $uploads = wp_upload_dir();
        $abs = trailingslashit( $uploads['basedir'] ) . ltrim( (string) $row['file_path'], '/' );
        if ( ! file_exists( $abs ) ) return new \WP_Error( 'file_missing', __( 'File missing on disk.', 'red-headed-lite' ), array( 'status' => 410 ) );
