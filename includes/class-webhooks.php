    ...
    protected static function dispatch( $event, $data ) {
        ...
        foreach ( $hooks as $h ) {
            ...
            $headers = array(
                ...
                'User-Agent'    => 'Red_Headed_Lite/' . RED_HEADED_VERSION,
                'X-Red_Headed_Lite-Event' => $event,
                'X-Red_Headed_Lite-Site'  => home_url(),
            );
            if ( ! empty( $h['secret'] ) ) {
                $headers['X-Red_Headed_Lite-Signature'] = 'sha256=' . hash_hmac( 'sha256', $body, $h['secret'] );
            }
