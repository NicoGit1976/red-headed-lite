...
class Red_Headed_Export_Engine {
    ...
    public static function run( $profile, $trigger_source = 'manual' ) {
        ...
        try {
            ...
            /* v1.5.0 — Empty-export guard: a 0-record run must NOT ship a ghost file.
               Skip every destination and log the job as 'empty'. */
            $is_empty  = ( count( $rows ) === 0 );
            $delivered = $is_empty ? null : self::deliver( $file, $profile, $format );

            /* v1.5.0 — Delivery visibility: a failed leg (SFTP auth/path…) must not
               hide behind a green "success" — mark 'partial' (error already logged to
               error_message by deliver()). */
            $had_failure = is_array( $delivered ) && (bool) array_filter(
                $delivered,
                function ( $d ) { return is_wp_error( isset( $d['ok'] ) ? $d['ok'] : null ); }
            );
            $status = $is_empty ? 'empty' : ( $had_failure ? 'partial' : 'success' );
    ...
    protected static function resolve_line_column( $order, $item, $key ) {
        ...
        switch ( $key ) {
            case 'line_sku':
                ...
                /* 4. Catalog re-link (v1.4.52): when a re-import recreates products under NEW
     ...
     * Returns the most-recent published product with that title that actually has
     * a SKU, or '' if none. Cached per request (one query per title).
     *
