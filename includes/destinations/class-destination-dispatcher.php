...
class Red_Headed_Destination_Dispatcher {
    public static function ship( $destination, $file, $profile, $format ) {
        ...
        switch ( $type ) {
            ...
            case 'local_zip':
                if ( Red_Headed_Soft_Lock::is_locked( 'dest_local_zip' ) ) return new \WP_Error( 'locked', __( 'Local ZIP requires Pro.', 'red-headed-lite' ) );
                return Red_Headed_Destination_Local_Zip::ship( $file, $destination );
            case 'local_folder':
                if ( Red_Headed_Soft_Lock::is_locked( 'dest_local_folder' ) ) return new \WP_Error( 'locked', __( 'Local folder destination requires Pro.', 'red-headed-lite' ) );
                return Red_Headed_Destination_Local_Folder::ship( $file, $destination );
            case 'rest':
                if ( Red_Headed_Soft_Lock::is_locked( 'dest_rest' ) ) return new \WP_Error( 'locked', __( 'REST destination requires Pro.', 'red-headed-lite' ) );
                return Red_Headed_Destination_REST::ship( $file, $destination );
            case 'gdrive':
                if ( Red_Headed_Soft_Lock::is_locked( 'dest_gdrive' ) ) return new \WP_Error( 'locked', __( 'Google Drive destination requires Pro.', 'red-headed-lite' ) );
            ...
            case 'download':
                ...
                return true;
            default:
                return new \WP_Error( 'unknown_destination', __( 'Unknown destination type.', 'red-headed-lite' ) );
        }
    }
