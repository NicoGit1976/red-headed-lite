...
class Red_Headed_Retry {
    ...
    public static function process() {
        ...
        foreach ( $q as $rec ) {
            ...
            if ( ! is_wp_error( $ok ) ) {
                self::note_job( (int) ( $rec['job_id'] ?? 0 ), sprintf( '[Red_Headed_Lite] retry OK after %d attempt(s) — %s', $rec['attempts'], $dest['type'] ?? '?' ) );
            ...
            if ( $max > 0 && $rec['attempts'] >= $max ) {
                self::note_job( (int) ( $rec['job_id'] ?? 0 ), sprintf( '[Red_Headed_Lite] retry GAVE UP after %d attempts — %s: %s', $rec['attempts'], $dest['type'] ?? '?', $rec['last_error'] ) );
