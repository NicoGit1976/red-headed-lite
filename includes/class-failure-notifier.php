...
class Red_Headed_Failure_Notifier {
    ...
    private static function send( $job_id, $profile, $error_message ) {
        ...
        $profile_name = is_array( $profile ) && ! empty( $profile['name'] ) ? $profile['name'] : sprintf( __( 'Profile #%d', 'red-headed-lite' ), is_array( $profile ) ? (int) ( $profile['id'] ?? 0 ) : 0 );
        ...
        $body    = sprintf(
            "%s\n\n%s: %s\n%s: %s\n%s: %d\n\n%s\n%s\n\n%s\n%s\n",
            __( 'A Red-Headed export job has failed.', 'red-headed-lite' ),
            __( 'Profile', 'red-headed-lite' ),    $profile_name,
            __( 'Site',    'red-headed-lite' ),    $site,
            __( 'Job ID',  'red-headed-lite' ),    $job_id,
            __( 'Error message:', 'red-headed-lite' ), $error_message,
            __( 'Open the Exports list:', 'red-headed-lite' ), $job_url
        );
